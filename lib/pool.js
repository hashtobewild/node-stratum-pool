...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        blockDiagnostics.DiagnoseSubmitBlockSent(rpcCommand, rpcArgs);

        _this.daemon.cmd(rpcCommand,
            rpcArgs,
            function (results) {
                blockDiagnostics.DiagnoseSubmitBlockResult(results);
    ...
    function GetBlockTemplate(callback) {
        ...
        function getBlockTemplate(subsidy) {
            ...
            [callConfig],
            function (result) {
                    // Log the getblocktemplate response... if needed
                    blockDiagnostics.DiagnoseGetBlockTemplateResult(result);
                    if (result.error) {
